...
def main():
    ...
    if is_master(args):
        ...
        logging.info(
            f'Total Model Parameters (M):\t        {round(sum(p.numel() for p in model_without_ddp.parameters()) / 1e6, 2)}')
        logging.info(
            f'Image Backbone Parameters (M):\t     {round(sum(p.numel() for p in model_without_ddp.image_backbone.parameters()) / 1e6, 2)}')
        logging.info(
            f'Text Backbone Parameters (M):\t      {round(sum(p.numel() for p in model_without_ddp.text_backbone.parameters()) / 1e6, 2)}')
        logging.info(
            f'Image Projection Parameters (M):\t   {round(sum(p.numel() for p in model_without_ddp.image_projection_head.parameters()) / 1e6, 2)}')
        logging.info(
            f'Text Projection Parameters (M):\t    {round(sum(p.numel() for p in model_without_ddp.text_projection_head.parameters()) / 1e6, 2)}')
