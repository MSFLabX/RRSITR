...
def train_one_epoch(
    ...
    if args.cache_teacher is not None and is_master(args):
        ...
        logging.info(
            f'Preparing to cache teacher features, size={cacher.feature.size()}, to be saved to {args.cache_teacher}')
    ...
    for i, batch in enumerate(dataloader):
        ...
        with autocast():
            with forward_context():
                ...
                image_features, text_features, logit_scale, text_tokens, image_tokens, local2= model(
                    images, texts, text_only=(args.cache_teacher is not None) or args.w_align == 0)  #


                if args.w_simcse > 0:
                    ssl_loss = args.w_simcse * simcse_contrastive_loss(image_features, text_features, local2,logit_scale=np.exp(2.996))
                else:
                    ssl_loss = 0
